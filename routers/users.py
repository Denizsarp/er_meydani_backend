...
async def search_users(db:Session=Depends(database.get_db), current_user:models.User = Depends(oauth2.get_current_user)):
    users = db.query(models.User).all()
    return users


@router.get('/{target_user_id}', status_code=status.HTTP_200_OK, response_model=schemas.UserDisplay)
async def get_target_user(target_user_id:UUID, db:Session=Depends(database.get_db), current_user:models.User = Depends(oauth2.get_current_user)):
...
async def delete_user(db: Session=Depends(database.get_db), current_user:models.User = Depends(oauth2.get_current_user)):
...
async def patch_user(new_features:schemas.UserUpdate, db:Session = Depends(database.get_db), current_user:models.User = Depends(oauth2.get_current_user)):
    target_user:schemas.User = current_user if current_user else None
    if not target_user.id == current_user.id:
        raise HTTPException(detail="No authentication", status_code=status.HTTP_401_UNAUTHORIZED)
